...
/* ---eseses----------------------set_database_connection_using_PDO---------------------- */
...
/* ------update_user_password------------------*/
	...
	public function update_user_password($data, $id){
		$employee_password  = $this->test_form_input_data(md5($data['user_password']));
...
/* -------------admin_password_change------------*/
	...
	private function send_task_email($user_id, $task_title, $task_description, $start_time, $end_time) {
		// Get user email
		$stmt = $this->db->prepare("SELECT email, fullname FROM tbl_admin WHERE user_id = :id");
		$stmt->execute(array(':id' => $user_id));
		...
		if($user) {
			try {
				...
				$mail->Port       = getenv('MAIL_PORT'); // SMTP port
				
				// Enable debugging
				$mail->SMTPDebug = 2; // Show client/server conversation
				$mail->Debugoutput = function($str, $level) {
				    error_log("SMTP: $str");
				};
				
				// Recipients
				$mail->setFrom(getenv('MAIL_USERNAME'), 'Task Management System');
	...
	public function add_new_task($data){
		...
		try{
			...
			$this->send_task_email($assign_to, $task_title, $task_description, $t_start_time, $t_end_time);
